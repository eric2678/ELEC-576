...
def weight_bias(shape, numOutputFilter, initialType="random"):
    W_conv = weight_variable(shape, type=initialType)
...
def convolutionalLayer(x_image, filterWidth, filterHeight, numInput, numOutputFilter, activateType, initialType):
...
def main_2c(x, y_, result_dir):
    ...
    autoCompute = -1
    h_pool2_flat = tf.reshape(h_pool2, [autoCompute, 7 * 7 * 64])
    W_fc1, b_fc1 = weight_bias([7 * 7 * 64, 1024], 1024, initialType=initializeType)
    ...
    # softmax
    W_fc2, b_fc2 = weight_bias([1024, 10], 10, initialType=initializeType)
